...
const Pricing: React.FC = () => {
  ...
  return (
    <section className="py-20 px-4 bg-gradient-to-b from-[#111111] via-[#0D0D0D] to-[#0D0D0D] min-h-screen relative z-20">
            ...
            >
              ...
              <div className="mb-6">
                <div className="flex items-center gap-2 mb-2">
                  <motion.h3
                    className={`text-2xl font-bold ${plan.name === 'Free' ? 'text-gray-400' : 'text-white'
                      }`}
                  ...
                  <motion.span
                    className={`text-xs px-2 py-1 rounded-full ${plan.name === 'Free'
                ...
                <div className="flex items-baseline">
                  <motion.span
                    className={`text-4xl font-bold ${plan.name === 'Free' ? 'text-gray-400' : 'text-white'
                      }`}
              ...
              <ul className={`space-y-3 ${plan.name === 'Free' ? 'mb-8 flex-grow' : 'mb-8'}`}>
                {plan.features.map((feature, featureIndex) => (
                  <motion.li
                    key={feature}
                    className={`flex items-start text-sm ${plan.name === 'Free' ? 'text-gray-500' : 'text-gray-300'
                      }`}
                    initial={{ opacity: 0, x: -30 }}
                    animate={cardsInView ? { opacity: 1, x: 0 } : { opacity: 0, x: -30 }}
                    transition={{ delay: cardsInView ? index * 0.25 + featureIndex * 0.12 + 0.8 : 0 }}
                  >
                    <div className={`w-5 h-5 rounded-full flex items-center justify-center mr-3 flex-shrink-0 mt-0.5 ${plan.name === 'Free'
                        ? 'bg-gray-600/20'
                        : 'bg-green-500/20'
                      }`}>
                      <svg className={`w-3 h-3 ${plan.name === 'Free' ? 'text-gray-500' : 'text-green-400'
